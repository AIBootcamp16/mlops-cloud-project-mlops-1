...
class CovidDataPreprocessor:
    ...
    def run(self, run_name: Optional[str] = None, from_latest: bool = False, input_artifact_uri: Optional[str] = None, input_csv_path: Optional[str] = None):
        """Airflow/CLI 표준 진입점"""
        ...
        with self.safe_mlflow_run(run_name):
            ...
            if raw_df is None and from_latest:
                print("[preprocess] Trying S3 direct search...")
                raw_df = self._load_from_s3_directly()
    ...
    def _load_from_s3_directly(self) -> Optional[pd.DataFrame]:
        """S3에서 직접 최신 CSV 파일 찾기"""
        try:
            import boto3
            from io import BytesIO
            from urllib.parse import urlparse

            artifact_root = os.getenv("MLFLOW_DEFAULT_ARTIFACT_ROOT", "s3://mlflow-dev-artifacts-team1-covid/mlflow-artifacts")
            parsed = urlparse(artifact_root)
            bucket = parsed.netloc
            prefix = parsed.path.lstrip("/")

            print(f"[preprocess] Searching S3: s3://{bucket}/{prefix}")
            ...
            for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
                for obj in page.get("Contents", []):
                    key = obj["Key"]
                    if key.endswith(".csv") and "korea_covid_raw" in key:
                        csv_files.append({"key": key, "last_modified": obj["LastModified"], "size": obj["Size"]})

            if not csv_files:
                print("[preprocess] No CSV files found in S3")
                return None

            latest = max(csv_files, key=lambda x: x["last_modified"])
            print(f"[preprocess] Found latest CSV: {latest['key']}")
            print(f"[preprocess]   - Modified: {latest['last_modified']}")
            print(f"[preprocess]   - Size: {latest['size']:,} bytes")

            obj = s3.get_object(Bucket=bucket, Key=latest["key"])
            df = pd.read_csv(BytesIO(obj["Body"].read()))

            print(f"[preprocess] ✅ Loaded {len(df)} rows from S3")
            ...
            print("[preprocess] boto3 not installed")
            return None
        except Exception as e:
            print(f"[preprocess] S3 direct load failed: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _load_from_latest_collection_safe(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            candidate_exp_names = ["covid_prediction_realtime", "covid_data_collection_v3.1", "covid_data_collection_v3", "covid_data_collection"]
            ...
            df = mlflow.search_runs(experiment_ids=exp_ids, order_by=["start_time DESC"], max_results=20)
            ...
            for _, row in df.iterrows():
                ...
                try:
                    artifact_paths = ["data", "raw_data", "artifacts"]
                    ...
                    for artifact_path in artifact_paths:
                        try:
                            ...
                            if csv_files:
                                ...
                                print(f"[preprocess] ✅ loaded from run {run_id[:8]}... (status={run_status}): {os.path.basename(csv_file)}")
                                return raw_df
                    ...
                    print(f"[preprocess] run {run_id[:8]}... failed: {e}")
                    continue

            print("[preprocess] No usable data found in any runs")
            return None
    ...
    def _create_dummy_data(self) -> pd.DataFrame:
        ...
        dummy_data = pd.DataFrame({
            'date': dates, 'country': 'South Korea',
    ...
    def _process_datetime_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"[datetime] Date range after validation: {data['date'].min().date()} ~ {data['date'].max().date()}")
    ...
    def _log_processing_results(self, raw_data: pd.DataFrame, processed_data: pd.DataFrame) -> None:
        ...
        dataset = mlflow.data.from_pandas(processed_data, source="covid_data_preprocessing", name="korea_covid_processed_dataset")
        mlflow.log_input(dataset, context="preprocessing")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        csv_filename = f"covid_processed_{timestamp}.csv"

        self.mlflow_manager.save_and_log_artifact(data=processed_data, filename=csv_filename, artifact_path="processed_data")
    ...
    def _create_preprocessing_metadata(self, raw_data: pd.DataFrame, processed_data: pd.DataFrame) -> Dict[str, Any]:
        """전처리 메타데이터 생성"""
        return {
            'preprocessing_info': {'timestamp': datetime.now().isoformat(), 'processor': 'MLflowCovidPreprocessor', 'version': 'v1.2_future_filter'},
            ...
            'processing_steps': ['datetime_feature_engineering_with_future_filter', 'high_missing_column_removal', 'cumulative_value_imputation', 'metadata_column_removal', 'categorical_encoding', 'linear_interpolation'],
            'data_quality': {
                'completeness_ratio': float(1 - (processed_data.isnull().sum().sum() / (processed_data.shape[0] * processed_data.shape[1]))),
...
def run_preprocessing_pipeline():
    ...
    processed_data = preprocessor.run(run_name="preprocessing_pipeline_v1.2_future_filter", from_latest=True)
