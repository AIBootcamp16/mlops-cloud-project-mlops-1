...
class IntegratedCovidTrainer:
    def __init__(self, cfg: Optional[TrainConfig] = None, tracking_uri: Optional[str] = None):
        ...
        self.models: Dict[str, Any] = {
            "random_forest": RandomForestRegressor(random_state=self.cfg.random_state, n_estimators=400, max_depth=12,
                                                   min_samples_leaf=2, n_jobs=-1),
            "gradient_boosting": GradientBoostingRegressor(random_state=self.cfg.random_state, n_estimators=300,
        ...
        with mlflow.start_run(run_name="model_training_integrated"):
            ...
            for name, model in self.models.items():
                try:
                    ...
                    results[name] = {"model": model, "metrics": {"train": m_tr, "test": m_te},
                                     "pred": {"train": ytr, "test": yte}}
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
                ...
                mlflow.log_artifact(f.name, "analysis/model_comparison.json");
                tmp = f.name
            os.remove(tmp)
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
                ...
                mlflow.log_artifact(f.name, "results/predictions_detailed.csv");
                tmp = f.name
            os.remove(tmp)
            ...
            try:
                if hasattr(best_model, "feature_importances_"):
                    imp = pd.DataFrame(
                        {"feature": X_train.columns, "importance": best_model.feature_importances_}).sort_values(
                        "importance", ascending=False)
                    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
                        imp.to_csv(f.name, index=False);
                        mlflow.log_artifact(f.name, "analysis/feature_importance.csv");
                        tmp = f.name
                    os.remove(tmp)
            except Exception:
                pass
            ...
            try:
                ...
                print(f"Model saved to: {info.model_uri}")

                # Registry 등록 시도 (항상 시도, 실패해도 계속 진행)
                try:
                    from mlflow.tracking import MlflowClient
                    client = MlflowClient()

                    # 모델 등록
                    mv = mlflow.register_model(info.model_uri, "covid_prediction_model")
                    print(f"Model registered as covid_prediction_model version {mv.version}")

                    # 자동으로 Production으로 전환
                    client.transition_model_version_stage(
                        name="covid_prediction_model",
                        version=mv.version,
                        stage="Production",
                    ...
                    print(f"Version {mv.version} promoted to Production stage")

                except Exception as reg_error:
                    mlflow.log_text(f"Registry registration failed: {reg_error}", "registry_error.txt")
                    print(f"Warning: Could not register model to registry: {reg_error}")
                    print("Model is still saved as artifact and can be used.")
            ...
            try:
                ...
                with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
                    ...
                    mlflow.log_artifact(f.name, "forecast/forecast_future.csv");
                    tmp = f.name
                os.remove(tmp)
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
                ...
                mlflow.log_artifact(f.name, "reports/training_summary.json");
                tmp = f.name
            os.remove(tmp)
