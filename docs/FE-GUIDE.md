...
데이터 누수(look-ahead/leakage) 방지: 예측 시점에 사용할 수 없는 정보 제거 → lag/rolling, trigonometric date features만 사용.

강건성(robustness): 파일 인코딩/구분자 자동 탐지, 잘못된 경로 보정, MLflow 서버 불능 시 로컬 파일 스토어로 자동 폴백.

실전성(practical): 최근 N일 고정 평가(set-aside) + (옵션) 시계열 교차검증, 동적 재귀 예측(H-step) 로 실제 운영 조건을 모사.
...
마지막 관측 이후 horizon일 만큼 날짜를 생성

각 미래일에 대해 직전까지의 예측값으로 lag/rolling/diff/pct 동적 업데이트 → 누수 없이 다단계 예측
...
add_lag_roll("new_cases") 적용 (결측 보정 포함):
...
미래 안전 피처만 선택: dow_*, month_*, weekofyear, dayofyear, new_cases_* 접두 열 → y_next, new_cases(당일값) 제외

수식/핵심 함수 해설
주기 임베딩 (누수 없이 계절성 인코딩)

요일/월은 범주형이지만 각도 공간의 연속성을 보존하기 위해 사인/코사인 사용:
...
미래일 t+1을 예측하면, 그 값을 다시 lag/roll/diff/pct 계산에 반영 → t+2 예측 … 반복

미래에서도 계산 가능한 구조를 코드로 강제하여 운영 일관성 확보

왜 이런 피처 구성이 성능에 유리한가?

누수 방지: 모델이 보지 못할 미래 데이터를 제거 → 실사용 성능과 오프라인 점수의 괴리 최소화

주기·계절성 반영: 전염병 데이터는 주간/월간 주기가 강함(보고 지연/휴일 효과 등) → 사인/코사인 임베딩으로 비선형 모델/선형 모델 모두에 유리

최근 추세 반영: lag/rolling/diff/pct은 최근 레벨/변동성/추세를 요약 → 급증/급감 포착

강건성: 선형 보간 + ffill/bfill로 학습 불안정(결측/이상) 최소화
...
시계열 CV: TimeSeriesSplit으로 데이터 누수 없는 검증. 분할 수(cv_splits)는 데이터 길이에 맞게 조정
...
# 학습 실행 (MLflow 서버 없으면 자동 폴백 → file:/root/mlruns)
...
로컬 파일 스토어 폴백 시: mlflow ui --backend-store-uri file:/root/mlruns --port 5001

서버(예: http://mlflow:8080) 사용 시: --tracking_uri 인자로 전달

체크리스트

 date 컬럼 정상 파싱(타임존/형식 확인)

 누수 방지: 당일/미래 데이터를 피처에 넣지 않았는가? (shift(1) 적용 확인)

 결측/이상치 보정(보간/ffill/bfill)으로 학습 안정성 확보

 test_days는 운영 관점(최근 기간)과 데이터 길이 밸런스에 맞는가?

 시계열 CV 분할 수가 충분하면서도 과도하지 않은가?

 예측 지평(H)을 늘릴수록 누적 오차를 고려해 창 길이나 모델 복잡도를 조정했는가?

FAQ

Q1. 왜 y_next를 쓰나요?
A. “다음날”을 예측 대상으로 명시해 one-step ahead 문제로 단순화합니다. H-step은 재귀로 확장합니다.

Q2. 왜 사인/코사인인가요?
A. 요일·월은 순환적 범주입니다. 원-핫보다 각도 공간 연속성을 주어 모델이 주기 구조를 더 잘 학습합니다.

Q3. rolling에서 shift(1)이 왜 필요하죠?
A. 당일 값을 포함하면 누수입니다. 항상 전일까지의 정보로만 창 통계를 만듭니다.

결론

본 피처 엔지니어링 구성은 미래 시점에서도 실제로 계산 가능한 정보만을 사용하도록 설계되어,
오프라인 점수와 운영 환경 성능의 일치를 최대화합니다.
주기성(요일/월), 최근 레벨·변동성(lag/rolling), 추세(diff/pct)을 결합하여
간결하면서도 강건한 베이스라인을 제공합니다. 여기에 도메인 피처(정책/이동/기상 등)를 future-aware 방식으로 확장하면 추가 성능 향상을 기대할 수 있습니다.
